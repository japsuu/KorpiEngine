...
public class MemoryEditor
{
    ...
    private static string FixedHex(int v, int count) => v.ToString("X").PadLeft(count, '0');
    ...
    {
        byte[] address = System.Text.Encoding.ASCII.GetBytes(input);
    ...
    public unsafe void Draw(string title, byte[] mem_data, int mem_size, int base_display_addr = 0)
    {
        ...
        ImGuiListClipper2 clipper = new ImGuiListClipper2(line_total_count, line_height);
        ...
        if (DataEditingAddr / Rows != data_editing_addr_backup / Rows)
        {
            ...
            bool scroll_desired = (scroll_offset < 0.0f && DataEditingAddr < visible_start_addr + Rows * 2) ||
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible items
        {
            ...
            for (int n = 0; n < Rows && addr < mem_size; n++, addr++)
            {
                ...
                if (DataEditingAddr == addr)
                {
                    ...
                    var flags = ImGuiInputTextFlags.CharsHexadecimal | ImGuiInputTextFlags.EnterReturnsTrue | ImGuiInputTextFlags.AutoSelectAll |
                                ImGuiInputTextFlags.NoHorizontalScroll | ImGuiInputTextFlags.CallbackAlways;
            ...
            // Draw ASCII values
            addr = line_i * Rows;
            StringBuilder asciiVal = new System.Text.StringBuilder(2 + Rows);
            asciiVal.Append("| ");
            for (int n = 0; n < Rows && addr < mem_size; n++, addr++)
            {
                int c = mem_data[addr];
        ...
        ImGui.Text(
            string.Format(
                " Range {0}..{1} ", FixedHex(base_display_addr, addr_digits_count),
