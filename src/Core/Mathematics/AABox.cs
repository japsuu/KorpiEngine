...
public partial struct AABox : ITransformable3D<AABox>
{
    ...
    public Vector3 CenterBottom => Center.SetZ(Min.Z);

    public Vector3[] Corners => GetCorners();
    ...
    public float Distance(Vector3 point) => Vector3.Zero.Max(Min - point).Max(point - Max).Length();
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public float CenterDistance(Vector3 point) => Center.Distance(point);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public AABox Translate(Vector3 offset) => new(Min + offset, Max + offset);


    public float DistanceToOrigin => Distance(Vector3.Zero);

    public float CenterDistanceToOrigin => CenterDistance(Vector3.Zero);

    public float Volume => IsEmpty ? 0 : Extent.ProductComponents();

    public Vector3 this[int n] => n == 0 ? Min : Max;

    public float MaxSide => Extent.MaxComponent();
    ...
    public float MinSide => Extent.MinComponent();

    public float Diagonal => Extent.Length();


    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ContainmentType Contains(AABox box)
    {
        // test if all corners are in the same side of a face by just checking min and max
    ...
    {
        Vector3 minVec = Vector3.MaxValue;
        Vector3 maxVec = Vector3.MinValue;
        if (points != null)
        {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static AABox Create(params Vector3[] points) => Create(points.AsEnumerable());


    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static AABox CreateFromSphere(Sphere sphere) => new(sphere.Center - new Vector3(sphere.Radius), sphere.Center + new Vector3(sphere.Radius));
    ...
    public PlaneIntersectionType Intersects(Plane plane)
    {
        ...
        // Inlined Vector3.Dot(plane.Normal, negativeVertex) + plane.D
        float distance = plane.Normal.X * bx + plane.Normal.Y * by + plane.Normal.Z * bz + plane.D;
        if (distance > 0)
            return PlaneIntersectionType.Front;

        // Inlined Vector3.Dot(plane.Normal, positiveVertex) + plane.D
    ...
    public static readonly AABox Unit = new(Vector3.Zero, new Vector3(1));
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 RelativePosition(Vector3 v) => v.InverseLerp(Min, Max);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public AABox Recenter() => Translate(-Center);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public AABox Scale(float scale) => new AABox(Recenter().Min * scale, Recenter().Max * scale).Translate(Center);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public IEnumerable<Vector3> GetCornersAndFaceCenters() => Corners.Concat(FaceCenters());
    ...
    /// </summary>
    /// <returns></returns>
    public Sphere ToSphere() => Sphere.Create(this);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 Lerp(Vector3 v) => Min + Extent * v;


    public AABox SetCenter(Vector3 v) => FromCenterAndExtent(v, Extent);

    public AABox SetExtent(Vector3 v) => FromCenterAndExtent(Center, v);

    public static AABox FromCenterAndExtent(Vector3 center, Vector3 extent) => new(center - extent / 2f, center + extent / 2f);

    public AABox Transform(Matrix4x4 mat) => Create(Corners.Select(v => v.Transform(mat)));
}