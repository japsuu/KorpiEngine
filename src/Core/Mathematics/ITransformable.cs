...
public interface ITransformable3D<out TSelf>
{
...
{
    public static Matrix4x4 Multiply(params Matrix4x4[] matrices) => matrices.Aggregate(Matrix4x4.Identity, (m1, m2) => m1 * m2);
    ...
    public static T Translate<T>(this ITransformable3D<T> self, float x, float y, float z) => self.Translate(new Vector3(x, y, z));
    ...
    public static T RotateX<T>(this ITransformable3D<T> self, float angle) => self.RotateAround(Vector3.UnitX, angle);
    public static T RotateY<T>(this ITransformable3D<T> self, float angle) => self.RotateAround(Vector3.UnitY, angle);
