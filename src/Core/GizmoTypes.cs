...
public class ArrowGizmo(Vector3 start, Vector3 end, ColorHDR color, float arrowHeadLength = 0.25f, float arrowHeadAngle = 20.0f) : Gizmo
{
    public override void Render(PrimitiveBatch batch, Matrix4x4 worldMatrix)
    {
        ...
        Vector3 right = Quaternion.CreateLookAtDirection(direction, Vector3.Up) * Quaternion.CreateFromEulerAnglesDegrees(0, 180 + arrowHeadAngle, 0) * new Vector3(0, 0, 1);
        Vector3 left = Quaternion.CreateLookAtDirection(direction, Vector3.Up) * Quaternion.CreateFromEulerAnglesDegrees(0, 180 - arrowHeadAngle, 0) * new Vector3(0, 0, 1);
