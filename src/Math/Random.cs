...
{
    public static uint RandomUInt(int index, int seed = 0) => (uint)Hash.Combine(seed, index);

    public static int RandomInt(int index, int seed = 0) => Hash.Combine(seed, index);

    public static float RandomFloat(float min, float max, int index, int seed) => (float)RandomUInt(index, seed) / uint.MaxValue * (max - min) + min;

    public static float RandomFloat(int index, int seed = 0) => RandomFloat(0, 1, index, seed);
